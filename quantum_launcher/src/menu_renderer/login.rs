...
use crate::{
    icon_manager,
    state::{
        AccountMessage, MenuLoginElyBy, MenuLoginMS, MenuLoginYggdrasil, Message, NEW_ACCOUNT_NAME,
    },
};

use super::{back_button, button_with_icon, Element};
impl MenuLoginYggdrasil {
    pub fn view(&self, tick_timer: usize) -> Element {
        let status: Element = if self.is_loading {
            ...
            button_with_icon(icon_manager::tick(), "Login", 16)
                .on_press(Message::Account(AccountMessage::YggdrasilLogin))
                .into()
        };
        ...
        let password_input = widget::text_input("Enter Password...", &self.password)
            .padding(padding)
            .on_input(|n| Message::Account(AccountMessage::YggdrasilPasswordInput(n)));
            ...
            widget::row![
                ...
                widget::column![
                    ...
                    widget::text_input("Enter Username/Email...", &self.username)
                        .padding(padding)
                        .on_input(|n| Message::Account(AccountMessage::YggdrasilUsernameInput(n))),
                    widget::text("Password:").size(12),
                    password_input,
                    widget::checkbox("Show Password", self.show_password)
                        .size(14)
                        .text_size(14)
                        .on_toggle(|t| Message::Account(AccountMessage::YggdrasilShowPassword(t))),
                    widget::text("Auth Server Url:").size(12),
                    widget::text_input(
                        "Enter auth server url... ie (https://drasl.example.com/authlib-injector)",
                        &self.url
                    )
                    .padding(padding)
                    .on_input(|n| Message::Account(AccountMessage::YggdrasilUrlInput(n))),
                    status,
                    widget::vertical_space(),
                ]
